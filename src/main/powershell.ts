...
// ─── Run an inline PowerShell script string ──────────────────────────────────
...
$protected = @('system','idle','smss','csrss','wininit','winlogon','lsass','lsaiso','services','svchost','registry','msmpeng','audiodg','dwm','fontdrvhost','ntoskrnl','spoolsv','searchindexer','trustedinstaller','wuauclt','taskhost','taskhostw','sihost','ctfmon','runtimebroker','securityhealthservice','securityhealthsystray','sgrmbroker','wmiprvse','conhost','dllhost','consent','msiexec','usoclient','sdclt')

# CPU sampling - two snapshots 500ms apart
$snap1 = @{}
...
$results = Get-Process -ErrorAction SilentlyContinue | Where-Object {
  ...
  $protected -notcontains $name -and $_.Id -ne 0
} | ForEach-Object {
  try {
    $proc = $_
    $pid2 = $proc.Id
    ...
    if ($snap1.ContainsKey($pid2)) {
      $delta = $proc.TotalProcessorTime.TotalMilliseconds - $snap1[$pid2]
      $numCPU = [Environment]::ProcessorCount
      if ($numCPU -lt 1) { $numCPU = 1 }
      $cpu = [math]::Round($delta / 5 / $numCPU, 2)
      if ($cpu -lt 0) { $cpu = 0 }
      if ($cpu -gt 100) { $cpu = 100 }
    }
    [PSCustomObject]@{ pid = $pid2; name = $name; priority = $priorityClass; cpu = $cpu }
  } catch {}
...
`
  ...
  ipcMain.handle('ps:batchOptimize', async (
    ...
    // Pre-filter on Node side (no need to do it in PS)
    ...
    // Build a JSON payload of PIDs → priority to set
    ...
    // Serialize pid list as PS array literal: @({"pid":1234,"priority":"High"},…)
    const psJsonArray = JSON.stringify(pidList)

    // Build IO pids array for the single-call IO block
